import { bracketData } from '../../constants';
import { BracketFormType, WallFormType } from '../../types';

export const bracketInterpolation = (
  wallType: string,
  wallData: WallFormType,

  brackets?: BracketFormType
) => {
  const {
    blockHeatConduction,
  ...
  const bracketArray = brackets!.bracket;
  ...
  const calculation = (wallValue: number, itemName: string) => {
    ...
    const insulationBorder = () => {
      ...
      const heatItem = bracketData.find((item) => item.name === itemName);
      if (heatItem) {
  ...
  const updatedBracketArray = bracketArray.map(
    ({ bracketBase, bracketName, bracketMaterial, bracketType, bracketQuantity }) => {
      const wallValue = () => {
        if (wallType === 'frame') {
          return bracketBase === 'concrete' ? concreteHeatConduction : blockHeatConduction;
      ...
      const final = calculation(wallValue(), bracketName);

      return {
        bracketConduction: final,
        bracketBase: bracketBase,
        bracketName: bracketName,
        bracketMaterial: bracketMaterial,
        bracketType: bracketType,
        bracketQuantity: bracketQuantity,
      };
    }
  );

  return { bracket: updatedBracketArray };
};
